...
func (p *Partition) WriteContents(f storage.WritableFile, contents io.Reader) (uint64, error) {
...
func (p *Partition) ReadContents(f storage.File, out io.Writer) (int64, error) {
	pss, lss := p.sectorSizes()
	total := int64(0)
	// chunks of physical sector size for efficient writing
	b := make([]byte, pss)
